...
def hook_model(model, image_f):
    ...
    def hook(layer):
        if layer == "input":
            ...
            assert layer in features, (f"Invalid layer {layer}. Retrieve the list of layers with "
                                       f"`lucent.modelzoo.util.get_model_layers(model)`.")
            out = features[layer].features
        assert out is not None, ("There are no saved feature maps. Make sure to put the model in eval mode, like so: "
    ...
    if c == 2:
        ...
        if points is not None:
            for i in range(b):
                ...
                # gt_masks[i,:,points[i,0]-5:points[i,0]+5,points[i,1]-5:points[i,1]+5] = torch.Tensor([255, 0,
                # 0]).to(dtype = torch.float32, device = torch.device('cuda:' + str(dev)))
                for pmt_id in range(p.shape[1]):
                    gt_masks[i, 0, p[i, pmt_id, 0] - 3:p[i, pmt_id, 0] + 3,
                    p[i, pmt_id, 1] - 3:p[i, pmt_id, 1] + 3] = 255
