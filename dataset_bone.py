...
class MRI_dataset(Dataset):
    def __init__(self, args, img_folder, mask_folder, img_list, phase='train', sample_num=50, channel_num=1, crop=False,
                 crop_size=1024, targets=['femur', 'hip'], part_list=['all'], cls=1, if_prompt=True,
                 prompt_type='point', region_type='largest_15', prompt_num=15, delete_empty_masks=False,
        ...
        if delete_empty_masks == 'delete' or delete_empty_masks == 'subsample':
            ...
            for idx, data in enumerate(self.data_list):
                ...
                if os.path.exists(os.path.join(self.mask_folder, mask_path)):
                    ...
                    msk = Image.open(
                        os.path.join(self.mask_folder.replace('2D-slices', '2D-slices-generated'), mask_path)).convert(
                        'L')
    ...
    def __getitem__(self, index):
        ...
        try:
            if os.path.exists(os.path.join(self.img_folder, img_path)):
                ...
                img = Image.open(
                    os.path.join(self.img_folder.replace('2D-slices', '2D-slices-generated'), img_path)).convert('RGB')
        except:
        ...
        if os.path.exists(os.path.join(self.mask_folder, mask_path)):
            ...
            msk = Image.open(
                os.path.join(self.mask_folder.replace('2D-slices', '2D-slices-generated'), mask_path)).convert('L')
        ...
        if self.phase == 'train':
            ...
            random_transform = OneOf([monai.transforms.RandGaussianNoise(prob=0.5, mean=0.0, std=0.1), \
                                      monai.transforms.RandKSpaceSpikeNoise(prob=0.5, intensity_range=None,
                                                                            channel_wise=True), \
